...
| 网络调试开关 | `echo 1 > /proc/sys/net/shanshui-guanxin/packet_trace`：环形日志打印每包**五元组 + 丢弃原因**（性能降 ~50%，仅供调试，替代 tcpdump） | ◻ | M13 | DESIGN §21.8 |
...
| 完整 /proc | /proc/self/{maps,status,exe,fd} + cpuinfo/mounts（**maps ✅ M13-01、status ✅ M13-02**） | ◻ | M13 | DESIGN §13.12 |
| 完整信号 | sigaction(SA_SIGINFO) + **SIGSEGV 投递**（用户态 #PF → handler/终止，✅ M13-06/10）、sigaltstack + 实时信号 | ◻ | M13 | DESIGN §13.10 |
