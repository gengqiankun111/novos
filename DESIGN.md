...
**基础生存型（v1.0 后 3~6 个月必提，关乎设备在真实环境"活下去"）**
...
**进阶生产力型（v1.5~v2.0 集中爆发）+ 生态扩展型（规模化部署后）**

| 需求 | 场景 | 方案 |
|---|---|---|
| 四层负载均衡（L4LB） | 多容器提供相同服务，入口流量轮询分发 | 基于 IPVS 的简单轮询，网关架构升级时考虑 |
| 流量镜像（Traffic Mirroring） | 复制一份流量送测试容器做灰度验证 | 网关转发层支持镜像端口 |

### 23.3 设计原则重申（所有新增需求的准绳）
