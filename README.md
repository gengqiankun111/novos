...
基于三大用户画像（嵌入式工程师 / 云原生 DevOps / 工业维护），真实高频新需求集中在
**硬件连接性 / 服务可靠性 / 远程可运维性**。原则：**只增不减**，作为 roadmap"定心丸"——
让早期用户敢于用 Novos 接真实项目（他们最怕的不是"功能没有"，而是"功能永远不会有"）。

| 紧迫度 | 版本 | 需求 |
|---|---|---|
| 基础生存型 | v1.1 | 容器保活策略（`restartPolicy`）、Web 管理界面默认开启 |
| | v1.5 | 4G/5G 蜂窝上网（PPP+USB 串口）、持久化日志落盘轮转、WireGuard VPN、存储卷独占锁、MicroPython、PTP/NTP 时间同步 |
| 进阶生产力型 | v2.0+ | 四层负载均衡（L4LB）、流量镜像 |

**设计原则**：内存优先（32MB/40MB 预算内验收）、默认禁用（显式开启）、模块化（feature flag 裁剪）。
详见 [DESIGN §23](docs/DESIGN.md)。
