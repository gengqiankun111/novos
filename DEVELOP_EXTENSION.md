...
| 阶段二（并行） | **`/top` HTTP 接口**：经 novos-gateway 暴露，返回 JSON（进程/CPU/内存数据），供 Web 前端展示 | DESIGN_EXT §3.2 | M14 gateway | Web 监控接口 | 浏览器访问 `/top` 返回合法 JSON，与 `top` 数据一致 |
...
| 阶段二 | M16-4 图形库（egui/fltk-rs）+ novos-monitor（与 top 共享 `/proc` 数据源） | DESIGN §13.16、§10.2 | M16-3 + M9 top | 可视化监视 | CPU/内存曲线实时刷新，与 top 数据一致 |
| 阶段三（v4.0） | M16-5 novos-fm 文件管理器（"文档/下载/桌面" + 盘符 C:/D: 映射 + 复制/移动/删除） | DESIGN §3.6、§19.3、§20.1 | M16-4 | 桌面应用 | 盘符视图与快捷入口可用；总内存 128–256MB |
