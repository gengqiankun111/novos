...
| M14-20 | HTTP 客户端增强：SSE 流式响应 + 长超时 + 大 JSON 流式解析（AI 调用 = HTTP 用例，DESIGN §18.6） | **P2** | M14-12 | 4 | 流式拉取云端 API 响应不整包缓冲 |
| M14-21 | （值得）Lua / MicroPython / QuickJS 轻量脚本运行时 | **P3** | M11-13 | 6 | 脚本容器内运行 hello 级验证 |
...
| 配置 | 关键路径 | 说明 |
|---|---|---|
| 1 人 | 约 14–17 个月 | 串行全部任务 + 20% 缓冲 |
| 2 人（主线 + M12 并行） | 约 9–12 个月 | 关键路径 268 − M12(27) ≈ 241 人日 ÷ 2 + M12 并行消化 + 20% 缓冲 |
